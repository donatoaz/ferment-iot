class ScheduleControlLoopsJob < ApplicationJob
  ...
  def perform
    ControlLoop.all.each do |cl|
      ...
      Rails.logger.debug "Checking #{cl.name} for run"
      if Time.now >= cl.next_run
        # enqueue assynchronous action, we don't want to hang around
        # here for too long
        ControlLoopJob.perform_later(cl.id)
        ActiveJob::Base.logger.info "Control Loop ##{cl.id}:#{cl.name} enqueued to run"
      	Rails.logger.debug "Control Loop #{cl.name} enqueued to run"
      end
      ...
      if Time.now >= cl.next_run + cl.sampling_rate.seconds
        ...
        Rails.logger.error "Controle Loop ##{cl.id}:#{cl.name} is missing deadlines!"
      end
    end
  end
end
